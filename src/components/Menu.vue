<template>
  <div class="hidden lg:block">
    <nav id="hello" class="flex flex-row bg-green-400 shadow-xl sticky top-0 z-30">
      <div class="ml-2 w-28 py-1 mr-4">
        <a href="#hello"><img alt="Grizzly Cogs" src="../assets/logo.png"></a>
      </div>
      <MenuItem item-title="Releases" link="#releases" :newtab="false" />
      <MenuItem item-title="Game soundtracks" link="#game-soundtracks" :newtab="false"/>
      ...
      <MenuItem item-title="Weekly Beats 2020-2021" :newtab="false" />
  ...
  <!-- open button -->
  <div class="lg:hidden fixed top-2 right-2 bg-green-400 px-2 py-1 rounded text-lg text-white" v-if="!menuOpened">
    <i class="fas fa-bars" v-on:click="menuOpened = true"></i>
  </div>
  <!-- close button -->
  <div class="lg:hidden fixed top-2 right-2 text-2xl text-green-400 z-50" v-if="menuOpened">
    <i class="fas fa-times" v-on:click="menuOpened = false"></i>
  </div>

  <div class="lg:hidden bg-white w-full h-full fixed bg-opacity-90 z-40 text-left pl-8 py-2 leading-10" v-if="menuOpened">
    <div class="text-center text-2xl text-green-400">Menu</div>
    <a href="#releases" class="text-green-400 text-2xl my-2" v-on:click="menuOpened=false">Releases</a><br />    
    <a href="#game-soundtracks" class="text-green-400 text-2xl my-2" v-on:click="menuOpened=false">Game soundtracks</a><br />
    ...
    <a class="text-green-400 text-2xl my-2" v-on:click="menuOpened=false">Weekly Beats 2020-2021</a><br />
